...
def tts_fn(
    ...
    try:
        ...
        import traceback
        logger.error(f"Full traceback for ValueError:\n{traceback.format_exc()}")
...
if __name__ == "__main__":
    ...
    # 수정: WebSocket 403 에러 해결을 위해 CORS 미들웨어 추가
    # Gradio의 user_middleware를 사용하여 launch() 전에 미들웨어 추가
    from starlette.middleware.cors import CORSMiddleware
    
    # launch() 전에 app.app이 있는지 확인하고 user_middleware에 추가
    if hasattr(app, 'app') and app.app is not None:
        try:
            gradio_app = app.app
            # user_middleware 속성에 CORS 미들웨어 추가
            if hasattr(gradio_app, 'user_middleware'):
                gradio_app.user_middleware.append(
                    (CORSMiddleware, {
                        'allow_origins': ["*"],
                ...
                logger.info("✅ Added CORS middleware to Gradio user_middleware (before launch)")
            # 또는 add_middleware 메서드 사용 시도
            elif hasattr(gradio_app, 'add_middleware'):
                try:
                    ...
                    logger.info("✅ Added CORS middleware to Gradio app (before launch)")
                except Exception as add_error:
                    logger.warning(f"⚠️  add_middleware failed: {add_error}, trying user_middleware")
                    # user_middleware에 직접 추가 시도
                    if not hasattr(gradio_app, 'user_middleware'):
                        gradio_app.user_middleware = []
                    gradio_app.user_middleware.append(
                        (CORSMiddleware, {
                            'allow_origins': ["*"],
                            'allow_credentials': True,
                    ...
                    logger.info("✅ Added CORS middleware via user_middleware list")
        except Exception as e:
            logger.warning(f"⚠️  Could not add CORS middleware before launch: {e}")
    
    # 수정: WebSocket 403 에러 해결을 위해 인증을 명시적으로 None으로 설정
    # launch() 실행
    app.launch(
        allowed_paths=['/file/images/'],
        server_name=args.server_name,
        share=args.share,
        inbrowser=not args.no_autolaunch,
        auth=None,  # 수정: 인증 비활성화 (WebSocket 403 에러 해결)
    )
