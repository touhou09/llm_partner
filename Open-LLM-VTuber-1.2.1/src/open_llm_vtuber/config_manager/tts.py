...
class BertVITS2TTSConfig(I18nMixin):
    ...
    line_split: bool = Field(True, alias="line_split")
    split_interval: float = Field(0.5, alias="split_interval")
    use_style_text: bool = Field(False, alias="use_style_text")
    style_text: str = Field("", alias="style_text")
    style_text_weight: float = Field(0.7, alias="style_text_weight")
    reference_audio_path: Optional[str] = Field(None, alias="reference_audio_path")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "length_scale": Description(en="Length scale (0.1-2)", zh="长度比例（0.1-2）"),
        "line_split": Description(en="Split text by line breaks", zh="按换行符分割文本"),
        "split_interval": Description(
            en="Interval between splits in seconds", zh="分割间隔（秒）"
        ),
        "use_style_text": Description(
            en="Use style text for guidance", zh="使用风格文本指导"
        ),
        "style_text": Description(en="Text for style guidance", zh="风格指导文本"),
        "style_text_weight": Description(
            en="Style text weight (0-1)", zh="风格文本权重（0-1）"
        ),
        "reference_audio_path": Description(
            en="Path to reference audio (optional)", zh="参考音频路径（可选）"
        ),
    }
...
class TTSConfig(I18nMixin):
    ...
    bert_vits2_tts: Optional[BertVITS2TTSConfig] = Field(None, alias="bert_vits2_tts")
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "bert_vits2_tts": Description(
            en="Configuration for Bert-VITS2 TTS", zh="Bert-VITS2 TTS 配置"
        ),
    }
    ...
    def check_tts_config(cls, values: "TTSConfig", info: ValidationInfo):
        ...
        elif tts_model == "bert_vits2_tts" and values.bert_vits2_tts is not None:
            values.bert_vits2_tts.model_validate(values.bert_vits2_tts.model_dump())
